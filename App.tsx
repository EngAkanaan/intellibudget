...
import { LayoutDashboard, Calendar, BarChart3, Wallet, Menu, X, Settings, Repeat, Target, HelpCircle, LogOut, Bot } from 'lucide-react';
...
type View = 'dashboard' | 'monthly' | 'yearly' | 'budgets' | 'recurring' | 'settings' | 'savings' | 'help' | 'agent';
...
const App: React.FC = () => {
  ...
  // Cross-device sync: whenever the user switches back to this tab, pull the
  // latest income sources and monthly data from Supabase.  This keeps desktop
  // and phone in sync without requiring Supabase Realtime.
  useEffect(() => {
    if (!user) return;

    const refresh = async () => {
      if (document.visibilityState !== 'visible') return;
      if (!dataLoaded) return; // initial load not done yet — skip

      try {
        const [freshMonthly, freshTemplates] = await Promise.all([
          monthlyDataApi.getAll().catch(() => null),
          incomeSourcesApi.getRecurringTemplates().catch(() => null),
        ]);

        if (freshMonthly) {
          const dbMap = new Map(freshMonthly.map(m => [m.month, m]));
          setData(prev => prev.map(m => dbMap.get(m.month) ?? m));
        }

        if (freshTemplates) {
          setRecurringIncomeTemplates(freshTemplates);
          recurringIncomeProcessedRef.current = '';
        }
      } catch {
        // Silently ignore refresh errors — user still sees cached data
      }
    };
  ...
  // Monthly summary email: fires once per calendar month on first login
  useEffect(() => {
    ...
    const key = `intellibudget_summary_${user.id}`;
    if (localStorage.getItem(key) === currentYM) return;

    localStorage.setItem(key, currentYM);
    supabase.functions
      .invoke('monthly-summary', { body: { month: prevYM } })
      .then(({ error }) => {
        if (error) console.warn('Monthly summary email skipped:', error);
      });
  }, [user?.id, dataLoaded]);
  ...
  // Detects when a newly added expense appears in 2+ prior months with a similar amount
  // and prompts the user to automate it as a recurring expense.
  const detectRecurringPattern = (expense: Omit<Expense, 'id'>, currentMonth: string) => {
    if (expense.recurringId || expense.amount <= 0) return;

    const priorMonths = data
      .map(m => m.month)
    ...
    const matchCount = priorMonths.filter(monthStr => {
      const monthData = data.find(m => m.month === monthStr);
      if (!monthData) return false;
      return monthData.expenses.some(
        e =>
          e.category === expense.category &&
          !e.recurringId &&
          Math.abs(e.amount - expense.amount) / expense.amount < 0.25
      );
    }).length;

    const alreadyRecurring = recurringExpenses.some(r => r.category === expense.category);

    if (matchCount >= 2 && !alreadyRecurring) {
      setTimeout(() => {
        if (
          window.confirm(
            `You've added a "${expense.category}" expense with a similar amount in the past ${matchCount} months.\n\nAutomate it as a recurring expense?`
          )
        ) {
          const dayOfMonth = parseInt(expense.date.split('-')[2]) || 1;
          addRecurringExpense({
  ...
  const addExpense = async (month: string, expense: Omit<Expense, 'id'>) => {
    ...
    const optimistic: Expense = { ...expense, id: tempId };

    // Optimistic: show immediately so the UI feels instant
    setData(prev =>
      prev.map(m =>
        m.month === month ? { ...m, expenses: [...m.expenses, optimistic] } : m
      )
    );
    ...
    try {
      ...
      setData(prev =>
        prev.map(m =>
          m.month === month
            ? { ...m, expenses: m.expenses.map(e => (e.id === tempId ? newExpense : e)) }
            : m
        )
      );
      detectRecurringPattern(expense, month);
    } catch (error) {
      // Revert on failure
      setData(prev =>
        prev.map(m =>
          m.month === month ? { ...m, expenses: m.expenses.filter(e => e.id !== tempId) } : m
        )
      );
  ...
  const deleteExpense = async (month: string, expenseId: string) => {
    // Capture the expense before removing it so we can restore on failure
    const targetMonth = data.find(m => m.month === month);
    const snapshot = targetMonth?.expenses.find(e => e.id === expenseId);

    // Optimistic: remove immediately
    setData(prev =>
      prev.map(m =>
        m.month === month ? { ...m, expenses: m.expenses.filter(e => e.id !== expenseId) } : m
      )
    );
    ...
    try {
      ...
      if (snapshot) {
        setData(prev =>
          prev.map(m =>
            m.month === month ? { ...m, expenses: [...m.expenses, snapshot] } : m
          )
        );
      }
  ...
  const deleteIncomeSource = async (month: string, incomeId: string) => {
    const targetMonth = data.find(m => m.month === month);
    const snapshot = (targetMonth?.incomeSources || []).find(i => i.id === incomeId);

    // Optimistic: remove immediately
    setData(prev =>
      prev.map(m =>
        m.month === month
          ? { ...m, incomeSources: (m.incomeSources || []).filter(i => i.id !== incomeId) }
          : m
      )
    );
    ...
    try {
      ...
      if (snapshot) {
        setData(prev =>
          prev.map(m =>
            m.month === month
              ? { ...m, incomeSources: [...(m.incomeSources || []), snapshot] }
              : m
          )
        );
      }
      console.error('Error deleting income source:', error);
      showToast('Failed to delete income source. Please try again.', 'error');
    }
  };

  // Deletes all instances of a recurring income series starting from `fromMonth`.
  // Past months are kept but their recurring metadata is stripped (so the
  // processRecurringIncome effect cannot resurrect the deleted future entries).
  const deleteRecurringIncomeFromMonth = async (recurringId: string, fromMonth: string) => {
    const snapshot = data.map(m => ({ month: m.month, incomeSources: m.incomeSources }));
    const templateSnapshot = [...recurringIncomeTemplates];

    // Optimistic — future/current months: remove entries entirely
    // Past months: strip recurring metadata so they're plain one-time records
    setData(prev =>
      prev.map(m => {
        if (m.month >= fromMonth) {
          return { ...m, incomeSources: (m.incomeSources || []).filter(i => i.recurringId !== recurringId) };
        }
        return {
          ...m,
          incomeSources: (m.incomeSources || []).map(i =>
            i.recurringId === recurringId
              ? { ...i, isRecurring: false, recurringId: undefined, recurringDayOfMonth: undefined, recurringStartDate: undefined }
              : i
          ),
        };
      })
    ...
    // Remove the template from state immediately so processRecurringIncome
    // does not find it and recreate the deleted rows before the API call finishes.
    setRecurringIncomeTemplates(prev => prev.filter(t => t.recurringId !== recurringId));
    recurringIncomeProcessedRef.current = ''; // allow effect to re-evaluate after change

    try {
      await incomeSourcesApi.deleteRecurringFromMonth(recurringId, fromMonth);
    } catch (error) {
      // Restore full state on failure
      setData(prev =>
        prev.map(m => {
          const s = snapshot.find(snap => snap.month === m.month);
          return s ? { ...m, incomeSources: s.incomeSources } : m;
        })
      );
      setRecurringIncomeTemplates(templateSnapshot);
      console.error('Error deleting recurring income:', error);
      showToast('Failed to delete recurring income. Please try again.', 'error');
    }
  };
  ...
  const addSavingsGoal = async (goal: Omit<SavingsGoal, 'id' | 'contributions'>) => {
    const tempId = `temp-${crypto.randomUUID()}`;
    const optimistic: SavingsGoal = { ...goal, id: tempId, contributions: [] };

    setSavingsGoals(prev => [...prev, optimistic]);
    try {
      ...
      showToast('Failed to add savings goal. Please try again.', 'error');
    }
  };

  const updateSavingsGoal = async (goal: SavingsGoal) => {
    const snapshot = savingsGoals.find(g => g.id === goal.id);

    // Optimistic: apply update immediately (important for contribution progress)
    ...
    try {
      ...
      if (snapshot) setSavingsGoals(prev => prev.map(g => (g.id === goal.id ? snapshot : g)));
      console.error('Error updating savings goal:', error);
      showToast('Failed to update savings goal. Please try again.', 'error');
    }
  };
  ...
  const deleteSavingsGoal = async (id: string) => {
    const snapshot = savingsGoals.find(g => g.id === id);

    setSavingsGoals(prev => prev.filter(g => g.id !== id));
    try {
      await savingsGoalsApi.delete(id);
    } catch (error) {
      if (snapshot) setSavingsGoals(prev => [...prev, snapshot]);
    ...
    <div className="flex min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200 font-sans">
      ...
      <nav className={`w-64 bg-gray-200 dark:bg-gray-700 flex flex-col fixed h-full shadow-2xl z-40 transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto min-h-0 scroll-smooth">
          <ul className="space-y-1 p-2">
            ...
            <NavItem view="agent" icon={<Bot />} label="AI Assistant" />
      ...
      {isAgentModalOpen && (
        <AgentChatModal
          ...
          onOpenFull={() => { setIsAgentModalOpen(false); setActiveView('agent'); }}
          addExpense={addExpense}
          addIncomeSource={addIncomeSource}
          addSavingsGoal={addSavingsGoal}
          addRecurringExpense={addRecurringExpense}
      ...
      <QuickNotesButton onToggle={() => setIsAgentModalOpen((o) => !o)} isOpen={isAgentModalOpen} />

      {/* Toast notification */}
      {toast && (
        <div
          className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 px-4 py-3 rounded-xl shadow-2xl text-white text-sm max-w-xs transition-all animate-fade-in ${
